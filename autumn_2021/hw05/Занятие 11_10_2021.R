...
# x.qq - список из векторов х и у:
# х - квантили нормального распределения
# y - исходная выборка, по которой высчитывались значения из х
x.qq <- qqnorm(z, plot.it = FALSE)

# пользуясь свойствами функции распределения, упорядочиваем х и у в х.qq
...
# z - данные, sort - функция, применяемая к подвыборке после ее получения

x.qqboot <- boot(z, sort, R = 999, 
                 sim = "parametric",ran.gen = x.gen)
sapply(1:2,function(i) lines(x.qq$x, x.qqboot$t[i,],
                               type = "l", col = "grey"))
