...
Xmat <- as.matrix(sleep_imp3[, -1])
M.las <- lars(Xmat, sleep_imp3[, 1], type = "lasso")
...
# Остатки модели
las.resid <- sleep_imp3$Sleep - predict.lars(M.las, Xmat, s = bestfrac,
                                           type ="fit", mode = "fraction")$fit
rss.lasso <- sum(las.resid^2)/(nrow(sleep_imp3) - 7)

# Гребневая регрессия
library(MASS)
M.ridge <- lm.ridge(Sleep ~ ., data = sleep_imp3, lambda = seq(0,2,0.1))

plot(x = M.ridge$lambda, y = M.ridge$GCV, type = "o")

(lambda <- M.ridge$GCV[which.min(M.ridge$GCV)])

(M.ridge1 <- lm.ridge(Sleep ~ ., data = sleep_imp3, lambda = lambda))
...
# Остатки модели
resid.ridge <- sleep_imp3$Sleep - beta.M.ridge1[1] - 
  as.matrix(sleep_imp3[,2:m])%*%beta.M.ridge1[2:m]

# Число степеней свободы гребневой регрессии
d <- svd(as.matrix(sleep_imp3[, 2:m]))$d
(df <- nrow(sleep_imp3) - sum(d^2/(lambda+d^2)))
